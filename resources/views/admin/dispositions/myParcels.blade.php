                    ...
                    @if (count($dispositions) > 0)
                        @foreach ($dispositions as $indexKey => $disposition)
                                <tr class="item{{$disposition->id}} {{ ($disposition->priority == 1) ? 'success' : '' }}" data-stock-time="{{$disposition->stock_time}}">
                                    ...
                                    <td>
                                        <a href="{{action('Admin\DispositionsController@driverReport', $disposition->id)}}" class="btn btn-{{ ($disposition->status == 3) ? 'warning' : 'info' }}"><span class="glyphicon glyphicon-edit"></span>{{ ($disposition->status == 3) ? ' Редагувати' : ' Створити' }}</a>
                                    </td>
                                </tr>
                        @endforeach
                    @else
